...
import { generateStableProjectId } from '../utils/formatters';
...
const ProjectList: React.FC<ProjectListProps> = ({ 
  ...
  selectedMonth, setSelectedMonth, userId
}) => {
  ...
  const handleGoogleSheetSync = async () => {
    ...
    try {
      ...
      if (importedProjects.length > 0) {
        const header = "Centro de custo,ID contabil,cliente,Projeto,UUID";
        const csvRows = importedProjects.map(p => `"${p.code}","${p.accountingId}","${p.client}","${p.name}","${p.id}"`);
        ...
        onImport(file);
        alert(`Sucesso!\n${importedProjects.length} operações do mês de ${sheetName} sincronizadas.`);
      } else {
    ...
    <div className="space-y-6">
       <div className="bg-white dark:bg-gray-800 border-2 border-green-500/30 p-5 rounded-2xl shadow-sm">
          <div className="flex items-center gap-3 mb-4">
             ...
             <div>
                <h3 className="font-bold text-gray-900 dark:text-white">Sincronização Incremental</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400">Preserva dados de meses anteriores</p>
             </div>
          </div>
       ...
       <div className="grid grid-cols-2 gap-4">
          <div className="bg-orange-50 dark:bg-orange-900/10 p-4 rounded-2xl border border-orange-200 dark:border-orange-800 text-center">
            <p className="text-xs font-bold text-orange-600 dark:text-orange-400 uppercase tracking-wider mb-1">Mês Atual ({MONTHS[selectedMonth]})</p>
            <p className="text-3xl font-black text-orange-700 dark:text-orange-300">{activeProjectsInMonth}</p>
          </div>
          <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl border border-gray-200 dark:border-gray-700 text-center flex flex-col justify-center items-center">
             <CheckCircle2 className="mb-1 text-green-500" size={20} />
             <p className="text-[10px] text-gray-500 uppercase font-bold">Base Persistente</p>
          </div>
       </div>
      ...
      {filteredProjects.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-700">
           ...
           <p className="text-gray-500 font-medium">Nenhuma operação para este mês.</p>
           <p className="text-xs text-gray-400 mt-1">Sincronize ou crie manualmente.</p>
        </div>
      ) : (
       ...
       <div className="pt-6">
        ...
        <div className="bg-gray-100 dark:bg-gray-700/50 p-4 rounded-2xl">
            <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-3 text-center">Importação Manual</p>
            ...
            >
                <Upload size={14} className="mr-2"/> Selecionar Planilha
            </button>
        </div>
    </div>
  );
