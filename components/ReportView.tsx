...
import { decimalHoursToHHMM, generateLegacyProjectId } from '../utils/formatters';
import { 
...
const COLORS = [
  '#c2410c', '#ea580c', '#f97316', '#fb923c', '#fdba74', '#6b7280', '#9ca3af', '#cbd5e1', '#d1d5db'
];

/**
 * Busca robusta: se não achar pelo ID direto, tenta reconstruir IDs antigos (com mês no hash).
 */
const findProjectGlobally = (id: string, allProjects: Project[], userId: string): Project | undefined => {
    const directMatch = allProjects.find(p => p.id === id);
    if (directMatch) return directMatch;
    for (const p of allProjects) {
        for (let m = 0; m < 12; m++) {
            if (generateLegacyProjectId(p.name, p.code, p.client, m, userId) === id) {
                return p;
            }
        }
    }
...
const getProjectDisplay = (project: Project | undefined) => {
    ...
    if (isNameNumeric && !isClientNumeric) {
        return { title: project.client, subtitle: `${project.name}` };
    }
    return { title: project.name, subtitle: `${project.client}` };
};

const ReportView: React.FC<ReportViewProps> = ({ projects, allocations, theme, userName, userId }) => {
  ...
  const { daysInMonth } = useMemo(() => {
    ...
    return { startDate: start, endDate: end, daysInMonth: eachDayOfInterval({ start, end }) };
  }, [date]);
  ...
  const matrixData = useMemo(() => {
    ...
    const rows = Array.from(relevantProjectIds).map(projectId => {
        const project = findProjectGlobally(projectId, projects, userId);
        ...
        return { projectId, project, display, dailyHours, totalHours };
  ...
  }, [daysInMonth, allocations, projects, userId]);
  ...
  const handleExport = () => exportDetailedMonthlyReportToExcel(projects, allocations, date, userName || '');
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-gray-800 p-2 border border-gray-200 dark:border-gray-700 rounded shadow text-sm z-50">
          ...
          <p className="text-orange-600 dark:text-orange-400">{decimalHoursToHHMM(payload[0].value)} horas</p>
        </div>
      );
    }
    return null;
    ...
    <div className="p-4 space-y-4 flex flex-col h-full pb-72">
      ...
      {matrixData.rows.length > 0 ? (
        <div className="flex flex-col space-y-8">
                    ...
                    <table className="w-full text-xs text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-b border-gray-300 dark:border-gray-600">
                                <th className="p-2 sticky left-0 bg-gray-100 dark:bg-gray-700 z-10 min-w-[150px] shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">Operação / Dia</th>
                                <th className="p-2 min-w-[60px] text-center font-bold bg-gray-200 dark:bg-gray-600 border-r border-gray-300 dark:border-gray-500">Total</th>
                                {daysInMonth.map(day => (
                            ...
                            {matrixData.rows.map((row) => (
                                <tr key={row.projectId} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors">
                                    <td className="p-2 sticky left-0 bg-white dark:bg-gray-800 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] max-w-[180px]">
                                        <div className="font-semibold text-gray-900 dark:text-gray-100 truncate">{row.display.title}</div>
                                        <div className="text-[10px] text-gray-500 dark:text-gray-400 truncate">{row.display.subtitle}</div>
                                    </td>
                                    <td className="p-2 text-center font-bold text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-gray-700/50 border-r border-gray-200 dark:border-gray-600">{decimalHoursToHHMM(row.totalHours)}</td>
                                    {row.dailyHours.map((hours, idx) => (
                                        <td key={idx} className={`p-1 text-center border-r border-gray-100 dark:border-gray-700 ${hours > 0 ? 'text-gray-900 dark:text-gray-100 bg-green-50 dark:bg-green-900/20' : 'text-gray-300 dark:text-gray-600'}`}>{hours > 0 ? decimalHoursToHHMM(hours) : '-'}</td>
                                    ))}
                                </tr>
                            ))}
                            <tr className="bg-gray-100 dark:bg-gray-700 font-bold border-t-2 border-gray-300 dark:border-gray-500">
                                <td className="p-2 sticky left-0 bg-gray-100 dark:bg-gray-700 z-10 text-right pr-4 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">TOTAL GERAL</td>
                                <td className="p-2 text-center text-orange-600 dark:text-orange-400 bg-gray-200 dark:bg-gray-600 border-r border-gray-300 dark:border-gray-500">{decimalHoursToHHMM(matrixData.monthGrandTotal)}</td>
                                {matrixData.dailyTotals.map((total, idx) => (
                                    <td key={idx} className="p-1 text-center text-[10px] text-gray-600 dark:text-gray-300 border-r border-gray-200 dark:border-gray-600">{total > 0 ? decimalHoursToHHMM(total) : ''}</td>
                                ))}
                            </tr>
                        </tbody>
                    </table>
             ...
             <button onClick={handleExport} className="w-full px-4 py-3 bg-green-600 text-white rounded-md hover:bg-green-500 transition-colors font-bold shadow-sm">Exportar para Excel (.xlsx)</button>
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-24">
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="h-[300px] md:h-[450px] w-full"> 
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={chartData.pieData} cx={isMobile ? "50%" : "40%"} cy="50%" labelLine={false} outerRadius={isMobile ? "65%" : "75%"} fill="#8884d8" dataKey="value" nameKey="name" label={({ cx, cy, midAngle, innerRadius, outerRadius, percent, index }) => {
                                        const radius = innerRadius + (outerRadius - innerRadius) * 0.5;
                                        const x = cx + radius * Math.cos(-midAngle * Math.PI / 180);
                                        const y = cy + radius * Math.sin(-midAngle * Math.PI / 180);
                                        const textColor = index >= 5 ? '#374151' : 'white';
                                        return percent > 0.05 ? (<text x={x} y={y} fill={textColor} textAnchor={x > cx ? 'start' : 'end'} dominantBaseline="central" fontSize={isMobile ? 10 : 12}>{`${(percent * 100).toFixed(0)}%`}</text>) : null;
                                    }}>
                                    {chartData.pieData.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}
                                </Pie>
                                <RechartsTooltip content={<CustomTooltip />} />
                                <Legend layout={isMobile ? "horizontal" : "vertical"} verticalAlign={isMobile ? "bottom" : "middle"} align={isMobile ? "center" : "right"} wrapperStyle={isMobile ? { fontSize: '11px', paddingTop: '10px', color: theme === 'dark' ? '#d1d5db' : '#000000' } : { fontSize: '12px', paddingLeft: '10px', color: theme === 'dark' ? '#d1d5db' : '#000000' }} />
                            </PieChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border border-gray-200 dark:border-gray-700 flex flex-col pb-16">
                    ...
                    <div className="h-[250px] md:h-[450px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart layout="vertical" data={chartData.topProjects} margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke={theme === 'dark' ? '#374151' : '#e5e7eb'} />
                                <XAxis type="number" hide />
                                <YAxis type="category" dataKey="name" width={isMobile ? 100 : 180} tick={{ fontSize: isMobile ? 9 : 11, fill: theme === 'dark' ? '#d1d5db' : '#000000' }} interval={0} />
                                <RechartsTooltip content={<CustomTooltip />} cursor={{fill: 'transparent'}} />
                                <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                                    {chartData.topProjects.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}
                                </Bar>
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
