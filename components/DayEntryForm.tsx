...
import { Plus, Trash2, Mic, Copy, Calculator, FastForward, CheckSquare, Square, Eraser, Loader2, AlertCircle } from 'lucide-react';
...
const DayEntryForm: React.FC<DayEntryFormProps> = ({ initialEntry, onSave, onReplicate, onDelete, projects, previousEntry }) => {
  ...
  const handleSaveClick = async (e: React.MouseEvent) => {
    ...
    setIsSaving(true);
    try {
        await onSave({ ...shifts, projectAllocations });
    } finally {
        setIsSaving(false);
    }
  };
  ...
  const handleCopyPreviousDay = () => {
      ...
      if (window.confirm("Copiar dados do dia anterior?")) {
          setShifts({
  ...
  const applyDistribution = () => {
      if (selectedProjectsForDist.length === 0 || totalWorkedHours === 0) return;
      const hoursPerProject = totalWorkedHours / selectedProjectsForDist.length;
      setProjectAllocations(selectedProjectsForDist.map(pid => ({ projectId: pid, hours: hoursPerProject })));
  ...
  const handleConfirmReplicate = () => {
      if (!onReplicate || selectedReplicationDates.length === 0) return;
  ...
  return (
    <>
      <div className="p-4 space-y-6">
             ...
             <div className="flex gap-2">
                {previousEntry && (
                    <button 
                        ...
                        className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-white rounded-md hover:bg-gray-300 transition-colors"
                    >
        ...
        <div className="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg space-y-4">
                  ...
                  return (
                    <div key={index} className="flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 p-3 rounded-md shadow-sm">
                        <div className="flex-1 min-w-0">
                            <p className="text-xl font-bold text-gray-900 dark:text-white leading-tight truncate">{display.title}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400 truncate mt-1">{display.subtitle}</p>
                        </div>
                        <input 
                            type="time" 
          ...
          <div className={`p-3 rounded-md text-center transition-colors ${
              ...
              <p className="font-semibold text-sm">
                  Apropriado: {decimalHoursToHHMM(totalAllocatedHours)} / {decimalHoursToHHMM(totalWorkedHours)}
              </p>
              { totalWorkedHours > 0 && !hoursMatch && <div className="flex items-center justify-center gap-1 text-[10px] mt-1"><AlertCircle size={10}/> Horas apropriadas e trabalhadas devem ser iguais.</div>}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
            <button 
                ...
                className="w-full px-4 py-3 bg-gray-500 text-white rounded-md font-semibold hover:bg-gray-600 flex items-center justify-center gap-2 text-sm"
            >
            ...
            <div className="flex gap-2">
                {onReplicate && hoursMatch && totalWorkedHours > 0 && (
                    <button
                        ...
                        className="flex-1 px-4 py-3 bg-cyan-700 text-white rounded-md font-semibold hover:bg-cyan-600 flex items-center justify-center gap-2 text-sm"
                    >
                ...
                <button 
                  ...
                  disabled={!hoursMatch || isSaving}
                  className="flex-1 px-4 py-3 bg-orange-600 text-white rounded-md font-semibold transition-colors disabled:bg-gray-400 dark:disabled:bg-gray-600 flex items-center justify-center text-sm"
                >
                  {isSaving ? <Loader2 size={18} className="animate-spin mr-2"/> : null}
                  {isSaving ? 'Gravando...' : 'Salvar Dia'}
                </button>
            </div>
        </div>
      </div>
      ...
      <Modal isOpen={isDistributeModalOpen} onClose={() => setIsDistributeModalOpen(false)} title="Distribuição Automática">
          <div className="flex flex-col h-[60vh]">
              <div className="mb-4">
                  <p className="text-xs text-gray-600 dark:text-gray-300 mb-2">Selecione projetos para dividir {decimalHoursToHHMM(totalWorkedHours)}.</p>
                  <input type="text" placeholder="Filtrar projetos..." value={distSearchTerm} onChange={(e) => setDistSearchTerm(e.target.value)} className="w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 text-sm"/>
              </div>
              <div className="flex-1 overflow-y-auto border rounded-md p-2 dark:bg-gray-900">
                  {filteredDistProjects.map(p => {
                      const isSelected = selectedProjectsForDist.includes(p.id);
                      const display = getProjectDisplay(p);
                      return (
                          <div key={p.id} onClick={() => toggleProjectSelection(p.id)} className={`flex items-start p-3 cursor-pointer rounded-md mb-2 transition-colors border ${isSelected ? 'bg-cyan-50 dark:bg-cyan-900/30 border-cyan-200' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700'}`}>
                             <div className="mt-1">{isSelected ? <CheckSquare className="text-cyan-600 mr-3" size={18}/> : <Square className="text-gray-400 mr-3" size={18}/>}</div>
                             <div className="min-w-0 flex-1">
                                 <p className="font-bold text-sm leading-tight">{display.title}</p>
                                 <p className="text-[10px] text-gray-500 mt-1">{display.subtitle}</p>
                             </div>
                          </div>
                      );
                  })}
              ...
              <div className="mt-4 pt-2 border-t flex justify-end gap-3">
                  <button onClick={() => setIsDistributeModalOpen(false)} className="px-4 py-2 text-sm">Cancelar</button>
                  <button onClick={applyDistribution} disabled={selectedProjectsForDist.length === 0} className="px-4 py-2 bg-cyan-600 text-white rounded-md disabled:opacity-50 text-sm">Distribuir</button>
              </div>
          </div>
      </Modal>

      <Modal isOpen={isReplicateModalOpen} onClose={() => setIsReplicateModalOpen(false)} title="Dias para Replicação">
          <div className="p-4 flex flex-col h-[70vh]">
              <div className="flex justify-between items-center mb-4">
                  <button onClick={() => setReplicationMonth(subMonths(replicationMonth, 1))} className="p-2">&lt;</button>
                  <span className="font-bold capitalize text-sm">{format(replicationMonth, 'MMMM yyyy', { locale: ptBR })}</span>
                  <button onClick={() => setReplicationMonth(addMonths(replicationMonth, 1))} className="p-2">&gt;</button>
              </div>
              <div className="grid grid-cols-7 gap-1 flex-1 overflow-y-auto">
                 {eachDayOfInterval({ start: startOfWeek(startOfMonth(replicationMonth)), end: endOfWeek(endOfMonth(replicationMonth)) }).map(day => {
                     const isSelected = selectedReplicationDates.some(d => isSameDay(d, day));
                     return (
                         <button key={day.toString()} onClick={() => toggleReplicationDate(day)} className={`p-2 rounded-md aspect-square flex items-center justify-center text-xs font-medium ${isSelected ? 'bg-cyan-600 text-white' : 'bg-gray-50 dark:bg-gray-800'}`}>{format(day, 'd')}</button>
                     );
                 })}
              </div>
              <div className="mt-4 pt-4 border-t">
                  <button onClick={handleConfirmReplicate} disabled={selectedReplicationDates.length === 0} className="w-full px-4 py-2 bg-cyan-700 text-white rounded-md disabled:opacity-50 text-sm">Confirmar ({selectedReplicationDates.length})</button>
              </div>
          </div>
      </Modal>
    </>
